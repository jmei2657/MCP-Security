...
if __name__ == "__main__":
    ...
    # 1) Try loading whatever's already there
    loaded_domains = load_stix_data(data_path)

    # 2) If we didn’t get enterprise, mobile AND ics, go fetch the missing bits
    needed = {"enterprise", "mobile", "ics"} - set(loaded_domains)
    if needed:
        print(f"[mcp] Missing STIX for: {needed}. Downloading now…")
        download_stix_data(data_path)
        # after download, reload everything
        loaded_domains = load_stix_data(data_path)

    # 3) If we STILL don’t have all three, bail out
    if set(loaded_domains) != {"enterprise", "mobile", "ics"}:
        print("[mcp] ERROR: failed to download/load all STIX domains.")
        exit(1)

    # 4) All good—start the server
    print(f"[mcp] Loaded STIX domains: {loaded_domains}. Starting FastMCP…")
    print("Running")
    mcp.run(transport="http", host="127.0.0.1", port=8001, path="/mitre-mcp")
